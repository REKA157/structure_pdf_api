...
def generate_structural_plan(structure, dimensions, ferraillage, output_path):
    ...
    # Exemples - à compléter avec des vraies proportions métier
    if structure == "poutre":
        L = dimensions.get("longueur", 5000) / 10
        H = dimensions.get("hauteur", 500) / 10
        msp.add_lwpolyline([(0, 0), (L, 0), (L, H), (0, H), (0, 0)], close=True)
        msp.add_text("Vue en plan / Top view").set_pos((L/4, -10))
        msp.add_text(f"{ferraillage.get('ha', 'HA12')} - {ferraillage.get('nb_barres', 4)} barres").set_pos((10, H+10))
        msp.add_text("Coupe A-A / Section").set_pos((0, H+30))
